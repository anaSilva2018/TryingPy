...
for ncmg in range(230, 410, 10):
    ...
    for ger in range (MPar.germax):
        ...
        [auxselect, auxselpop, auxselvol] = _cSelect(MPar.pop, nPeriod, auxmut, auxcost, auxvol, auxpot)
        auxnewpop = auxselect.mvsel
        ger_bestcost[0, ger] = auxselect.mcsel[0, 0]
        ger_bestcmg[0, ger] = auxcost.mcmg[0, 0]
        ger_Phidr[:, ger] = auxpot.phidr[:, 0]
        ger_Pterm[:, ger] = auxpot.pterm[:, 0]
        ger_vturb[:, ger] = auxvol.vturb[:, 0]
        ger_vsobr[:, ger] = auxvol.vsobr[:, 0]
